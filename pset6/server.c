...
main (int argc, char *argv[])
{
	...
	if (connected ())
	{
		...
		*queryPtr = '\0';
	    ...
	    path =
		malloc (sizeof (char) *
			(strlen (argv[argc - 1]) + strlen (requesttarget) +
			 1));
		...
		char *format =
		    "QUERY_STRING=\"%s\" REDIRECT_STATUS=200 SCRIPT_FILENAME=\"%s\" php-cgi";
		char command[strlen (format) + (strlen (path) - 2) +
			     (strlen (query) - 2) + 1];
...
error (unsigned short code)
{
    ...
    char content[strlen (template) + 2 * ((int) log10 (code) + 1 - 2) +
		 2 * (strlen (phrase) - 2) + 1];
 ...
 */
const char *
lookup (const char *extension)
{
    // convert to lowercase first
    char *ext[] = { "css", "html", "gif", "ico", "jpg", "js", "png" };
    char *mime[] =
	{ "text/css", "text/html", "image/gif", "image/x-icon", "image/jpeg",
	"text/javascript", "image/png"
    };

    for (int i = 0; i < 7; i++)
    {
	if (strcasecmp (extension, ext[i]) == 0)
	{
	    return mime[i];
	}
    }
...
parse (void)
{
	...
	char *needle =
	    memmem (haystack, request + length - haystack, "\r\n\r\n", 4);
	if (needle != NULL)
	{
 ...
 */
void
start (short port, const char *path)
{
